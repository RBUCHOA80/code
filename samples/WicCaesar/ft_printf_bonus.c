...
/*   Updated: 2022/08/26 20:09:13 by cnascime         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	raiseflags(const char *exp, t_flags *flags)
{
	...
	{
		flags->minwidth = ft_atoiforprintf(&exp[flags->i]);
		flags->i += ft_places(flags->minwidth);
	}
	if (exp[flags->i] == '.')
	{
		flags->precision = ft_atoiforprintf(&exp[++flags->i]);
		flags->i += ft_places(flags->precision);
	}
}
