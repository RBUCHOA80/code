...
/*   Updated: 2022/07/30 12:00:55 by ruchoa           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (read(fd, 0, 0) < 0 || BUFFER_SIZE < 1 || FOPEN_MAX < fd)
		return (NULL);
	next_line[fd] = ft_get_buffer(next_line[fd], fd);
	if (!next_line[fd])
		return (NULL);
	actual_line = ft_actual_line(next_line[fd]);
